...
-(BOOL)performDragOperation:(id<NSDraggingInfo>)sender {
    ...
    NSURL *fileURL = [NSURL fileURLWithPath:pboardArray.firstObject];
    if ([self.delegate respondsToSelector:@selector(dragAcceptView:draggedFilesWithURL:)]) {
        [self.delegate dragAcceptView:self draggedFilesWithURL:fileURL];
    }
    return YES;
}
@end
