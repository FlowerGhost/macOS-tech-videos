...
- (void)viewDidLoad {
    ...
    for (VideoURLModel *model in results) {
        [self.dataArr addObject:[NSURL fileURLWithPath:model.urlPath]];
    }
    if ([[NSUserDefaults standardUserDefaults]objectForKey:KCurrentURLPath] && self.dataArr.count > 0) {
        _currentPlayingURL = [NSURL fileURLWithPath:[[NSUserDefaults standardUserDefaults]objectForKey:KCurrentURLPath]];
        [self createAudioPlayerWithURL:_currentPlayingURL];
    }
...
-(void)createAudioPlayerWithURL:(NSURL *)url {
    ...
    [[NSUserDefaults standardUserDefaults]setObject:url.path forKey:KCurrentURLPath];
    if (self.item) {
        [self removeObserver];
        self.item = nil;
        self.player = nil;
...
-(void)openFile {
    ...
    if (result == NSModalResponseOK) {
        ...
        self.selectedFileURL = self.dataArr.firstObject;
        for (NSURL *url in self.dataArr) {
...
-(void)dragAcceptView:(DragAcceptView *)dragView draggedFilesWithURL:(NSURL *)url {
